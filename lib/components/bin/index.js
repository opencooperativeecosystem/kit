...
export const NewBin = ({children, clicked, touched, errors, name, note, toggleClicked, scope, options, dateComponent}) => {
  ...
  <Wrapper>
    ...
    ? <BinContainer>
      <MainHeader>
        {/* <Input name={name} placeholder='Type a process name' />
        { touched.name && errors.name && <Alert>{errors.name}</Alert> }
        <WrapperTextArea><Textarea name={note} placeholder='Write the process description' /></WrapperTextArea>
        { touched.note && errors.note && <Alert>{errors.note}</Alert> }
        <Select name={scope}>{options.map((opt) => opt)}</Select>
        { touched.scope && errors.scope && <Alert>{errors.scope}</Alert> }
        <DateWrapper>
          <H5>Due</H5>
          {dateComponent}
        </DateWrapper> */}
